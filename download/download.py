...
from os import chdir  # для смены рабочей папки
    ...
    def db_upload(self):
        ...
        while True:  # "Бесконечный" цикл
            ...
            for elem in posts:  # Перечисление постов
                ...
                db.execute(f'''
                INSERT INTO main(og,edited,length,link,img_id,thread,date,views,likes,reposts,popularity)
                VALUES ("{text}","{edited}",{length},"{link}",{img_id},{thread},{date},{int(views)},{int(likes)},{int(reps)},{popularity})''')
                # Загрузка в БД
